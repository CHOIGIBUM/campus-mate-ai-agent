...
**대학생 공모전 정보를 수집·구조화하고,<br/>개인화 추천부터 Notion 승인·Slack 브리핑·Google Calendar 반영까지 연결하는 AI Agent Harness**
...
대회에서는 Timely를 중심으로 Python 스크립트와 LLM, Notion·Slack·Google Calendar 커넥터를 연결해 공고 수집부터 일정 반영까지의 전체 흐름을 시연했습니다. 이후 당시 구현을 정리하면서 Agent와 Skill의 역할을 명확히 하고, 실행 코드와 테스트를 보완해 공개 가능한 형태로 다듬었습니다.
...
Campus Mate는 Agent와 Skill이 작업 순서와 검증 기준을 정의하고, Python 코드가 수집·파싱·추천·외부 서비스 연동을 실제로 수행하도록 구성했습니다.
...
Agent는 각 단계에서 무엇을 확인하고 다음 단계로 넘길지를 정합니다. Python 코드는 수집, 파싱, 점수 계산, API 연동처럼 반복 가능해야 하는 작업을 담당합니다.
...
Timely에서는 위 여섯 역할을 세 가지 반복 작업으로 묶어 운영했습니다.
...
각 Skill에는 호출 조건, 입력과 출력, 검증 기준, 실패 처리와 실제 Python 명령을 정리했습니다. 세부 역할은 [`role-table.md`](./role-table.md)에서 확인할 수 있습니다.
...
- 정기 수집 과정에서는 `Accept`, `Hold`, `Reject`, `Scheduled` 상태를 덮어쓰지 않습니다.
- Slack은 추천 공고를 전달하는 채널이며, 참가 승인은 Notion에서 처리합니다.
- Google Calendar 일정은 `Accept` 상태인 공고에만 생성합니다.
- 일부 일정 생성에 실패하면 성공한 event ID는 보존하고, 실패한 항목만 다시 시도합니다.
...
Google Calendar 일정은 Python이 중복 방지 정보가 포함된 요청 목록을 만들고, Timely/Composio가 실제 이벤트를 생성한 뒤, 그 결과를 다시 Python에 전달해 Notion 상태를 갱신하는 방식입니다. 이벤트별 성공과 실패를 따로 기록하므로 실패한 항목만 다시 시도할 수 있습니다.
